...
static bool build_cached_decode_ffn_graph(
    ...
    if (hash_routed) {
        ...
        if (moe_hybrid_graph_policy().fused_combine) {
            routed_out = ggml_moe_combine(out.sg.ctx, down_e, weights);
        } else {
            ggml_tensor * weights_3d = ggml_reshape_3d(
                out.sg.ctx, weights, 1, n_used, n_tokens);
            ...
            routed_out = ggml_reshape_2d(
                out.sg.ctx, routed_out, w.n_embd, n_tokens);
        }
...
static ggml_tensor * build_moe_ffn(
    ...
    if (layer_idx < w.n_hash_layer && L.ffn_gate_tid2eid) {
        ...
        if (moe_hybrid_graph_policy().fused_combine) {
            routed_out = ggml_moe_combine(
                ctx, down_e, routing.weights);
        } else {
            ggml_tensor * weights_3d = ggml_reshape_3d(
                ctx, routing.weights, 1, n_used, n_tokens);
...
static ggml_tensor * ds4_build_hash_routed_ffn(
    ...
    ggml_tensor * routed_out = nullptr;
    if (moe_hybrid_graph_policy().fused_combine) {
        routed_out = ggml_moe_combine(ctx, down_e, weights);
    } else {
        ggml_tensor * weights_3d = ggml_reshape_3d(
